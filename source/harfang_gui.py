...
class HarfangUI:
	...
	def new_single_widget(cls, type):
		...
		widget.update({
			...
			"sub_widgets": [], # Sub-widgets are widgets generated by the widget (e.g: listbox, generates text_select widgets)
			"child_mouse_click": False, # If mouse click on a sub widget 
			"states": []
		})
		return widget
...
# ------------ Widgets system
	...
	@classmethod
	# !!! Separate this function in specific component type callbacks !!!
	def update_component(cls, widget, component):
		...
		if component["type"] == "list_box":
			widget_id = widget["name"]
			n = 0
			y_pos = 0
			max_width = 0
			for item_name in widget["items_list"]:
				ts_id = widget_id + ".ts##" + str(n)
				mc, _ = cls.text_select(ts_id, item_name, n == widget["current_idx"], cursor_auto = False, forced_text_width = component["forced_text_width"])
				if mc:
					widget["child_mouse_click"] = True
				...
				if primitive["type"] in stackable_primitives:
					...
					if primitive["type"] == "texture":
						...
						if primitive["textures"] is not None:
							cls.set_toggle_idx(primitive, primitive["textures"], widget["toggle_idx"])
							tsx, tsy = primitive["texture_size"].x, primitive["texture_size"].y
							tsx *= primitive["texture_scale"].x
							tsy *= primitive["texture_scale"].y
						...
						if "toggle_texts" in widget:
							...
							if widget["toggle_texts"] != primitive["texts"]:
								...
								primitive["texts_d"] = []
								if primitive["texts"] is not None and len(primitive["texts"]) > 0:
									for i in range(len(primitive["texts"])):
										primitive["texts_d"].append(hg.Vec2(0, 0))
									mx = 0
									for text in primitive["texts"]:
										ts = HarfangGUIRenderer.compute_text_size(cls.current_font_id, text)
										primitive["texts_sizes"].append(ts)
										mx = max(mx, ts.x)
						...
						if primitive["texts"] is not None:
							...
							#Component "forced_text_width" overwrites primitive var:
	...
	# ------------ Toggle mecanisms
	@classmethod
	def set_toggle_idx(cls, obj, toggle_elements_list, new_idx):
		idx_clamp = min(len(toggle_elements_list)-1, new_idx)
		if obj["toggle_idx"] != idx_clamp:
			obj["toggle_idx_start"] = obj["toggle_idx"]
			obj["toggle_idx"] = idx_clamp
			obj["toggle_t0"] = cls.timestamp
	...
	def toggle_button(cls, widget_id, texts: list, current_idx, **args):
		...
		obj_toggle_comp = widget["objects_dict"]["toggle_button_box"]
		obj_label = widget["objects_dict"]["basic_label.1"]
	...
	def text_select(cls, widget_id, text:str, selected: bool, **args):
		...
		if obj_text["forced_text_width"] is not None:
	...
	def list_box(cls, widget_id, current_idx: int, items_list: list, **args):
		...
		if "line_space_factor" in args:
			lb_component["line_space_factor"] = args["line_space_factor"]
		obj_label = widget["objects_dict"]["basic_label.1"]
		obj_label["text"] = cls.get_label_from_id(widget_id)
		if "show_label" in args:
			widget["components"]["basic_label"]["hidden"] = not args["show_label"]
		...
		if "forced_text_width" in args:
			lb_component["forced_text_width"] = args["forced_text_width"]
	...
	def dropdown(cls, widget_id, current_idx:int, items_list:list, **args):
		...
		if "forced_text_width" in args:
			obj_toggle_comp["forced_text_width"] = args["forced_text_width"]
			obj_list_box["forced_text_width"] = args["forced_text_width"]
